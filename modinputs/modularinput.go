...
// Returns the number of bytes written, an error if anythin went wrong
...
func (mi *ModularInput) Run() (err error) {
	...
	if len(os.Args) == 1 {
		...
		if n, err = mi.LoadConfigFromStdin(); err != nil {
			mi.Log("FATAL", fmt.Sprintf("Errow when loading configurations from StdIn. Read %d bytes. Error: %s", n, err.Error()))
			return err
		}
