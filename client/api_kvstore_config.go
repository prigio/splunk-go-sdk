...
*/
func (entry *collectionEntry[KVStoreCollResource]) Query(ss *SplunkService, query, fields, sort string, limit, skip int, shared bool, storeJSONResultInto *[]map[string]interface{}) error {
	if storeJSONResultInto == nil {
		return fmt.Errorf("kvstore[%s] query: argument storeJSONResultInto cannot be nil", entry.Name)
	}
...
func (entry *collectionEntry[KVStoreCollResource]) Insert(ss *SplunkService, jsondata string) (key string, err error) {
	dataURL := strings.ReplaceAll(entry.Links.List, "/collections/config/", "/collections/data/")
	...
	if err = doSplunkdHttpRequest(ss, "POST", dataURL, nil, []byte(jsondata), "application/json", &dataRes); err != nil {
		return "", fmt.Errorf("kvstore[%s] insert: %w", entry.Name, err)
	}
...
func NewKVStoreCollCollection(ss *SplunkService) *KVStoreCollCollection {
